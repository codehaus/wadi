...
public class CountingEnvelopeInfoCombiner implements InvocationResultCombiner {
    ...
    public InvocationResult combine(Collection<InvocationResult> invocationResults) {
        Set<CountingEnvelopeInfo> combined = new HashSet<CountingEnvelopeInfo>();
        for (InvocationResult result : invocationResults) {
            CountingEnvelopeInfo info = (CountingEnvelopeInfo) result.getResult();
            if (null == info) {
                continue;
            }
            combined.add(info);
        }
        return new InvocationResult(combined);
    }
