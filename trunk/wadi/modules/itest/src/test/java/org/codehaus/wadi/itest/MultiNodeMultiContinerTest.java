...
public class MultiNodeMultiContinerTest extends TestCase {
	...
	public void testSimpleReplication() throws Exception {
        ...
        for(int i = 0;i < nodes.length;i++) {
            methods[i] = new GetMethod("http://localhost:" + decorator.getPortForNode(nodes[i]));
        }
        // there are no sessions so these should not return 200
        for(int i = 0;i < methods.length;i++) {
            assertNotSame(new Integer(200), new Integer(get(client, methods[i], "/wadi-test;jsessionid=foo")));
            methods[i] = null;
        }
        ...
        for(int i = 0;i < methods.length;i++) {
            assertEquals(new Integer(200), new Integer(get(client, methods[i], "/wadi-test")));
            methods[i] = null;
        }
		...
		assertEquals(new Integer(200), new Integer(get(client, methods[0], "/wadi-test/set.jsp")));
        for(int i = 1;i < methods.length;i++) {
            assertEquals(new Integer(200), new Integer(get(client, methods[i], "/wadi-test/check.jsp")));
            assertNotSame(new Integer(-1), new Integer(methods[i].getResponseBodyAsString().indexOf("foo = bar")));
            methods[i] = null;
        }
	}
