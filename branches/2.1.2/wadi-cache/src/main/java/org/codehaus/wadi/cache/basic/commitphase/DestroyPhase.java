 ...
 * @version $Rev:$ $Date:$
 */
public class DestroyPhase implements CommitPhase {
    private final Manager manager;

    public DestroyPhase(Manager manager) {
        if (null == manager) {
    ...
    public void execute(Map<Object, CacheEntry> keyToEntry) throws TransactionException {
        for (Map.Entry<Object, CacheEntry> entry : keyToEntry.entrySet()) {
            ...
            if (cacheEntry.getState() == CacheEntryState.DELETED) {
                try {
                    manager.contextualise(new DestroyCacheEntryInvocation(key, AcquisitionInfo.EXCLUSIVE_LOCAL_INFO));
