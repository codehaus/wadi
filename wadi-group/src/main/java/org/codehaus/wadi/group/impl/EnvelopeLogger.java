...
public class EnvelopeLogger extends NoOpEnvelopeInterceptor {
    ...
    private Envelope onEnvelope(Envelope envelope, String type) {
        if (log.isTraceEnabled()) {
            log.trace(type + ": Address [" + envelope.getAddress() + "]; ReplyTo [" + envelope.getReplyTo()
                    + "]; TargetCorrelationId [" + envelope.getTargetCorrelationId() + "]; SourceCorrelationId ["
                    + envelope.getSourceCorrelationId() + "]; Payload [" + envelope.getPayload() + "]");
        }
        return envelope;
    }
